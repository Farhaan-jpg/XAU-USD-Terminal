...
export async function chart(symbol, interval, range, ttl = 30000) {
  ...
  for (const host of HOSTS) {
    try {
      ...
      // Weekend / off-hours fallback: if range is 1d and candles are empty, attempt 5d range
      if (!candles.length && range === "1d") {
        try {
          const fallbackUrl = `https://${host}/v8/finance/chart/${encodeURIComponent(symbol)}?interval=${interval}&range=5d`;
          const fbData = await fetchJson(fallbackUrl);
          const fbR = fbData?.chart?.result?.[0];
          if (fbR) {
            const fbQ = fbR.indicators?.quote?.[0] || {};
            candles = (fbR.timestamp || [])
              .map((t, i) => ({
                t: t * 1000,
                o: fbQ.open?.[i] ?? null,
                h: fbQ.high?.[i] ?? null,
