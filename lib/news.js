...
const parser = new Parser({
  timeout: 18000,
  headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", Accept: "application/rss+xml, application/xml, text/xml;q=0.9, */*;q=0.8" },
  customFields: { item: [["dc:date", "dcDate"], ["content:encoded", "contentEncoded"]] },
});
...
async function fetchFeed(url) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (!feed || !Array.isArray(feed.items) || !feed.items.length) throw new Error("empty feed");
      const mapped = (feed.items || []).map((it) => ({
        title: he.decode(it.title || "").trim(),
        link: it.link || "",
        pubDate: it.isoDate || it.pubDate || null,
        content: he.decode(it.contentSnippet || "").trim(),
      }));
      feedCache.set(url, mapped);
      return mapped;
    } catch (e) {
      lastErr = e;
...
export async function news(refresh = false) {
  ...
  for (const url of config.goldFeeds) {
    ...
    for (const it of items) {
      const text = (it.title + " " + it.content).slice(0, 600);
      ...
      const dedupeKey = slug(it.title).slice(0, 60);
