...
export async function bias(refresh = false) {
  ...
  const futuresPrice = dailyCloses.length ? last(dailyCloses) : null;
  const basePrice = futuresPrice ?? spotPrice ?? null;
  const spread = (futuresPrice != null && spotPrice != null) ? (futuresPrice - spotPrice) : 0;
  ...
  let spotPivots = null, spotPrevDay = null;
  if (daily) {
    ...
    if (ds.length >= 2) {
      ...
      if (spread !== 0) {
        spotPrevDay = { high: prev.h - spread, low: prev.l - spread, close: prev.c - spread, t: prev.t };
        spotPivots = pivotLevels(prev.h - spread, prev.l - spread, prev.c - spread);
      } else {
        spotPrevDay = prevDay;
        spotPivots = pivots;
      }
  ...
  const result = {
    ...
    indicators: {
      ...
      spotVwap: vw != null && spread ? round(vw - spread, 2) : (vw != null ? round(vw, 2) : null),
    ...
    levels: {
      ...
      spot: {
        price: spotPrice,
        vwap: vw != null ? round(vw - spread, 2) : null,
        pivots: spotPivots,
        prevDay: spotPrevDay,
        dayHigh: sessHigh != null ? round(sessHigh - spread, 2) : null,
        dayLow: sessLow != null ? round(sessLow - spread, 2) : null,
        swingS: swingS != null ? round(swingS - spread, 2) : null,
        swingR: swingR != null ? round(swingR - spread, 2) : null,
      },
      futures: {
        price: futuresPrice,
        vwap: vw != null ? round(vw, 2) : null,
